...
// The Charts namespace
$A.Charts = $A.Charts ? error(16, true) : {};
 ...
 * First, it deep merged arrays, which lead to workarounds in Charts. Second,
...
(function ($) {
	/**
	 * The default AuroraAdapter for jQuery
	 */
	win.AuroraAdapter = win.AuroraAdapter || ($ && {
		...
		init: function (pathAnim) {
			...
			// extend some methods to check for elem.attr, which means it is a Charts SVG object
			...
			/**
			 * Register Charts as a plugin in the respective framework
			 */
			$.fn.charts = function () {
				...
				if (options !== UNDEFINED) {
					...
					chart = new $A.Charts[constr](options, args[1]);
					ret = this;
					/*jslint unused:true*/
				}
				...
				if (options === UNDEFINED) {
					ret = charts[attr(this[0], 'data-aurora-chart')];
				}	
		...
		fireEvent: function (el, type, eventArguments, defaultFunction) {
			...
			// Remove warnings in Chrome when accessing layerX and layerY. Although Charts
...
// check for a custom AuroraAdapter defined prior to this file
var globalAdapter = win.AuroraAdapter,
...
// Utility functions. If the AuroraAdapter is not defined, adapter is an empty object
...
defaultOptions = {
	...
	global: {
		useUTC: true,
		//timezoneOffset: 0,
		canvasToolsURL: 'Canvas-tools-min.js',
	...
	credits: {
		enabled: false,
		text: 'Aurora',
		href: 'http://aurora.hand-china.com',
		position: {
			align: 'right',
			x: -10,
			verticalAlign: 'bottom',
...
SVGRenderer.prototype = {
	...
	/**
	 * Create a group
	 * @param {String} name The group will be given a class name of 'aurora-{name}'.
 ...
 */
$A.Charts.VMLElement = VMLElement = {
...
$A.Charts.VMLRenderer = VMLRenderer = function () {
	this.init.apply(this, arguments);
};
...
if (useCanVG) {
	 ...
	 */
	$A.Charts.CanVGRenderer = CanVGRenderer = function () {
 ...
 */
var Pointer = $A.Charts.Pointer = function (chart, options) {
...
Pointer.prototype = {
	...
	onDocumentMouseMove: function (e) {
		...
		if (chartPosition && hoverSeries && !this.inClass(e.target, 'aurora-tracker') &&
	...
	onContainerMouseMove: function (e) {
		...
		if ((this.inClass(e.target, 'aurora-tracker') || 
...
var TrackerMixin = $A.Charts.TrackerMixin = {
 ...
 */
var Legend = $A.Charts.Legend = function (chart, options) {
	this.init(chart, options);
};
...
var LegendSymbolMixin = $A.Charts.LegendSymbolMixin = {
	 ...
	 * symbols through $A.Charts.seriesTypes[type].prototype.drawLegendSymbols.
	 * 
	 * @param {Object} legend The legend object
	 */
...
Chart.prototype = {
	...
	getContainer: function () {
		var chart = this,
			...
			chartHeight,
			renderTo,
			indexAttrName = 'data-aurora-chart',
			oldChartIndex,
			containerId;
	...
	destroy: function () {
		...
		chart.renderTo.removeAttribute('data-aurora-chart');
...
//    	}else{
			...
			for(var j=0;j<sf.yAxis.length;j++){
				var yAxis=sf.yAxis[j].options,
					...
					name = yAxis.name,
					yAxisNames;
				if(linkedTo || !name)continue;
				yAxisNames = name.split(',');
				if(yAxis.bindtarget){
...
//    	}
    ...
    bind : function(ds){
        ...
        this.dataset = ds;
        this.processDataSetLiestener('on',this);
        this.onRedraw();
    }
}; // end Chart
...
var CenteredSeriesMixin = $A.Charts.CenteredSeriesMixin = {
	/**
...
Series.prototype = {
	...
	setData: function (data, redraw) {
		...
		if (turboThreshold && dataLength > turboThreshold) {
			...
			if (isNumber(firstPoint)) { // assume all points are numbers
				...
				error(12); // Charts expects configs to be numbers or arrays in turbo mode
			}
		} else {
			for (i = 0; i < dataLength; i++) {
	 ...
	 * unused points and optionally grouped in Charts Stock.
	 */
